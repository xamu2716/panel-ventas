...
**Deployments acumulados (normal, no es un problema):** cada push deja un deployment nuevo e inmutable en el dashboard de Vercel; solo el más reciente queda activo en el dominio de producción, los anteriores no consumen nada ni afectan el rendimiento mientras están quietos. El plan Hobby no tiene tope al número total de deployments guardados (los límites son de uso: ancho de banda, horas de función, minutos de build al mes). Si molesta que las URLs viejas (`panel-ventas-xxxxx.vercel.app`) sigan accesibles, se pueden borrar a mano desde el dashboard (menú de tres puntos → Delete) o con `vercel remove <url>`, o configurar limpieza automática en Project Settings → General → Deployment Retention.
